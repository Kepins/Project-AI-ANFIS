...
class Rocket:
    ...
    def scale_to_power(percent_of_screen):
        t = Constants.percent_top
        b = Constants.percent_bot
...
class Simulation:
    ...
    def run():
        ...
        cursor_set = False
        while not cursor_set:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    return False
                elif event.type == pygame.MOUSEBUTTONDOWN:
                    x, y = pygame.mouse.get_pos()
                    r = Constants.rect_to_start
                    if r.x < x < (r.x + r.width) and r.y < y < (r.y + r.height):
                        cursor_set = True
        pygame.mouse.set_visible(True)
        pygame.mouse.set_pos(Constants.pos_mouse_start)

        # start simulation
        while Simulation.rocket.height > 0:
            # input
            x, y = pygame.mouse.get_pos()
            percent_of_screen = (Constants.screen_size[1] - y) / Constants.screen_size[1]
            normalized_power = Rocket.scale_to_power(percent_of_screen)
            frames.append([Simulation.rocket.height, Simulation.rocket.vel, normalized_power])
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    return False
            ...
            time += Constants.dt
            Simulation.rocket.update(normalized_power)
            ...
            power = Simulation.font.render('power: ' + str(round(normalized_power, 2)), False, (0, 0, 0))
            ...
            Simulation.screen.blit(power, (0, Constants.font_size*2))
            Simulation.screen.blit(fps, (0, Constants.font_size*3))
            Simulation.draw_power(normalized_power)
            Simulation.rocket.draw(Simulation.screen)
